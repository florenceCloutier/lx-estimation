...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    left_displacement = (2 * np.pi * robot_spec['wheel_radius'] / robot_spec['encoder_resolution']) * left_encoder_ticks
    right_displacement = (2 * np.pi * robot_spec['wheel_radius'] / robot_spec['encoder_resolution']) * right_encoder_ticks

    delta_dist = (left_displacement + right_displacement) / 2
    delta_theta = (right_displacement - left_displacement) / robot_spec['wheel_baseline']
    ...
    v = delta_dist * np.cos(phi_max)  # replace this with a function that uses the encoder
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = int(belief.shape[0] - (d_t[i, j] - grid_spec['d_min']) / grid_spec['delta_d']) # replace with something that accounts for the movement of the robot
